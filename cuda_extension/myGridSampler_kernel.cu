...
__global__ void my_grid_sampler_2d_kernel(
    ...
    CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
        ...
        scalar_t x = grid[n][h][w][0];
        scalar_t y = grid[n][h][w][1];
        ...
        if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            for (index_t c = 0; c < C; ++c) {
                if (within_bounds_2d(iy_nw, ix_nw, inp_H, inp_W)) {
                    output[n][c][h][w] += input[n][c][iy_nw][ix_nw] * nw;
                }
                if (within_bounds_2d(iy_ne, ix_ne, inp_H, inp_W)) {
                    output[n][c][h][w] += input[n][c][iy_ne][ix_ne] * ne;
                }
                if (within_bounds_2d(iy_sw, ix_sw, inp_H, inp_W)) {
                    output[n][c][h][w] += input[n][c][iy_sw][ix_sw] * sw;
                }
                if (within_bounds_2d(iy_se, ix_se, inp_H, inp_W)) {
                    output[n][c][h][w] += input[n][c][iy_se][ix_se] * se;
                }
            }
            ...
            for (index_t c = 0; c < C; ++c) {
                if (within_bounds_2d(iy_nearest, ix_nearest, inp_H, inp_W)) {
                    output[n][c][h][w] = input[n][c][iy_nearest][ix_nearest];
                } else {
...
template<typename scalar_t, typename index_t>
__global__ void my_grid_sampler_2d_backward_kernel(
        const index_t nthreads,
    ...
    CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
        ...
        if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            for (index_t c = 0; c < C; ++c) {
                ...
                // calculate grad_grid
                if (within_bounds_2d(iy_nw, ix_nw, inp_H, inp_W)) {
                    scalar_t nw_val = input[n][c][iy_nw][ix_nw];
                    gix -= nw_val * (iy_se - iy) * gOut;
                    giy -= nw_val * (ix_se - ix) * gOut;
                }
                if (within_bounds_2d(iy_ne, ix_ne, inp_H, inp_W)) {
                    scalar_t ne_val = input[n][c][iy_ne][ix_ne];
                    gix += ne_val * (iy_sw - iy) * gOut;
                    giy -= ne_val * (ix - ix_sw) * gOut;
                }
                if (within_bounds_2d(iy_sw, ix_sw, inp_H, inp_W)) {
                    scalar_t sw_val = input[n][c][iy_sw][ix_sw];
                    gix -= sw_val * (iy - iy_ne) * gOut;
                    giy += sw_val * (ix_ne - ix) * gOut;
                }
                if (within_bounds_2d(iy_se, ix_se, inp_H, inp_W)) {
                    scalar_t se_val = input[n][c][iy_se][ix_se];
            ...
            grad_grid[n][h][w][0] = gix_mult * gix;
            grad_grid[n][h][w][1] = giy_mult * giy;
        } else if (interpolation_mode == GridSamplerInterpolation::Nearest) {
            if (input_requires_grad) {
                index_t ix_nearest = static_cast<index_t>(::round(ix));
                index_t iy_nearest = static_cast<index_t>(::round(iy));
            ...
            //   1. use index with gGrid_sW to directly compute gGrid_ptr_NHW
            //   2. directly assign to gGrid_ptr_NHW[0], gGrid_ptr_NHW[1]
            grad_grid[n][h][w][0] = static_cast<scalar_t>(0);
            grad_grid[n][h][w][1] = static_cast<scalar_t>(0);
        }
    }
}
...
//    globalContext().alertNotDeterministic("my_grid_sampler_2d_backward_cuda");
    ...
    int64_t count = N * H * W;
    if (count > 0) {
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "my_grid_sampler_2d_backward_kernel", [&] {
            my_grid_sampler_2d_backward_kernel<scalar_t>
            <<<GET_BLOCKS(count, 1024), 1024>>>(
