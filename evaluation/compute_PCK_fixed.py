...
def detect_pose_from_generated(gen_iterator):
    ...
    pred_pose = []
    # imgs = []
    for i, (img, pose_2d) in enumerate(tqdm(gen_iterator)):
        with torch.no_grad():
            # color range is [0, 255]
            img = (img[:, :, :, ::-1] * 127.5 + 127.5).astype("uint8")
            # imgs.append(img)

            for p2, im in zip(pose_2d, img):
                pose_results = detect_pose(im.transpose(1, 2, 0))

                gt_pose.append(p2 * np.array([[-1, 1]]) + np.array([[128, 0]]))
                pred_pose.append(pose_results)
        # if i == 30:
        #     break
    # import pickle
    # with open("tmp_pose.pkl", "wb") as f:
    #     pickle.dump({"imgs": imgs,
    #                  "pred_pose": pred_pose,
    #                  "gt_pose": gt_pose}, f)
    # exit()
...
def pck(joint_error):
    # import pdb
    # pdb.set_trace()
    pckh = np.mean(joint_error < 1.5, axis=0)
    ...
    path = f"{out_dir}/result/{out_name}/pckh_{suffix}_fixed.npy"
    np.save(path, pckh)
    print(path, pckh)

    path = f"{out_dir}/result/{out_name}/joint_error_{suffix}_fixed.npy"
    np.save(path, joint_error)
