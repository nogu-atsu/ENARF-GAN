...
class TriPlaneNeRF(NeRF):
    ...
    def coarse_to_fine_sample(self, image_coord: torch.tensor, pose_to_camera: torch.tensor,
        ...
        with torch.no_grad():
            ...
            if depth_min.shape[-1] > 0:
                ...
                local_coarse_points, canonical_coarse_points = self.to_local_and_canonical(coarse_points,
                                                                                           pose_to_camera,
                                                                                           bone_length)
                # coarse density
    ...
    def render(self, image_coord: torch.tensor, pose_to_camera: torch.tensor, inv_intrinsics: torch.tensor,
        ...
        if fine_depth is not None:
            ...
            local_fine_points, canonical_fine_points = self.to_local_and_canonical(fine_points, pose_to_camera,
                                                                                   bone_length)
            ...
            if not self.training:
                self.temporal_state.update({
                    "canonical_fine_points": canonical_fine_points,
                    "in_cube": in_cube(local_fine_points),
                    "tri_plane_feature": tri_plane_feature,
                })
